...
function verifyJWT(req,res,next){
    ...
    jwt.verify(token,process.env.JWT_SECRET_KEY,(err,decoded)=>{
        ...
        const authorizedRoutes = {
            ...
            "/api/edit-userRole/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i": {"action" : "edit", "resource" : "User"},
            "/api/get-allUsers" : {"action": "view", "resource": "User"},
            "/api/delete-user/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i" : {"action": "delete", "resource": "User"},
            // Routes for TvGroup
            ...
            "/tvgroups/:groupID/tvs/^tv\d{6}$/i": {"action" : "delete"},
            "/tvgroups/:groupID/tvs/batch-delete": {"action" : "delete", "resource": "Tv"},
            "/tvgroups/:groupID/tvs/batch-update": {"action": "edit", "resource": "Tv"},

            // Advertisements 
            "/advertisements" : {"action" : "view", "resource": "Advertisement"}, // Retrieve all advertisements 
            "/api/delete/^\d{13}$/i" : {"action" : "delete", "resource": "Advertisment"}, // Delete ads by ad id 
            "/advertisements/^\d{13}$/i" : {"action": "view", "resource": "Advertisement"},
            "/api/update-ad" : {"action": "edit", "resource": "Advertisement"},
            "/advertisements/:adID/^\d{13}$/i" : {"action": "view", "resource": "Advertisement"},
            // Template editor routes 
            "/api/update-coordinates": {"action": "edit", "resource": "Template"}
